...
The setup baseline verifies that one official Qwen Q4_K_M GGUF can generate a response on CPU. The repeatable three-variant benchmark is a separate experiment.

For a command-by-command explanation and tested CMD/Git Bash equivalents, read [the setup walkthrough](../docs/setup_walkthrough.md).

## Windows baseline setup

Run these commands in PowerShell from the repository root. `winget install llama.cpp` is the simple official option where WinGet exists. On this machine it was absent, so the following pinned official CPU release was used instead.
...
The baseline run returned `Paris` and exited successfully. Two earlier raw runs are also retained: the first answer hit its 64-token cap, and the second returned `15` for `5 + 2 * 3` (correct value `11`). This small model's mistake is a useful reason to use objective checks in the later comparison. `llama.cpp` displayed prompt and generation rates during these runs, but they are not treated as comparable benchmark findings.

The benchmark harness uses identical prompt text and generation settings for FP16, Q8_0, and Q4_K_M, retains full responses, and captures the CLI's prompt and generation rates. Each record includes the backend build, model file size, and runtime settings; model hashes are documented in `progress/llm_benchmark.md`. Machine details and run-order limitations are documented alongside the results.

## Benchmark preparation checks
