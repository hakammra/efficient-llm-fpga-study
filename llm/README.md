...
`runner.py` contains `run_one(item, model_name, run_number)`, which launches one inference, saves the raw transcript and structured record, and uses a `day02_bench_` filename prefix so batch runs stay separate from pilot runs. Fully saved runs are skipped on restart; a partial saved run stops for inspection. `benchmark.py` validates the 20 prompts and three model paths; the prompt/model loop is the next hands-on step before any batch inference starts.
