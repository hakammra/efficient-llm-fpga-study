# LLM benchmark data collection — complete

## Goals

- Obtain the official Qwen2.5-0.5B-Instruct FP16 and Q8_0 GGUF files alongside the setup baseline's Q4_K_M file.
...
- Audited all 60 records with zero issues, documented measured metrics and their limits, and updated the repository overview for analysis.
...
- Q4_K_M GGUF was verified during setup. These are file properties, not inference performance results.
...
- The single Q4_K_M `math_01` pilot record reports response `42`, exact check `true`, exit code 0, whole-process time 4.759 s, prompt rate 76.3 tokens/s, and generation rate 24.3 tokens/s. These are one-run observations from `results/raw/pilot_initial_q4_math_01_record.json`, not comparative statistics. Earlier manual runs of the same pilot reported whole-process times 2.476 s and 5.283 s, illustrating run-to-run variation.
...
- The pilot required manually selecting one model and one prompt. The full loop now covers all pairs; `runner.py` separates its output files from pilots with a `benchmark_` prefix, skips fully saved runs on restart, and stops on incomplete saved runs for inspection.
